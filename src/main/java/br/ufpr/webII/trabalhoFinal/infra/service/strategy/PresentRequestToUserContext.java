    ...
    public String showRequestToUser(String profile){
        return switch (profile) {
            ...
            case "Employee" -> """
                ...
                JOIN public.address a ON c.address_id = a.id
                WHERE r.id IN (
                    SELECT request_id FROM public.request_status rs
                    WHERE (status = 'OPEN' OR in_charge_employee_id = ?)
                    AND date_time = (SELECT MAX(date_time) FROM public.request_status
                    WHERE request_id = rs.request_id AND in_charge_employee_id IS NOT NULL)
                ) AND r.active = TRUE;
                """;
